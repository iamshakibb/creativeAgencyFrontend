...
export default function ServiceSection() {
  ...
  return (
    <Container>
      <Row
        className="services my-5 align-items-center justify-content-center"
        id="services"
      >
          ...
          <Row>
            {!sixServices ? (
              ...
              sixServices.map((service) => (
                <div
                  className="col-md-4 servicesItem text-center"
                  key={service._id}
                >
                  {/* <Link to={{ pathname: `/order/${service.title}`, search: `?sort=${service.image.img}` }}> */}
                  <Link
                    to={{
                      pathname: `/order/${service.title}`,
                      search: `?sort=${service.image.img}`,
                    }}
                  >
