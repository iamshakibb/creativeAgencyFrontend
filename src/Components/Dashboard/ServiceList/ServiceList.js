...
export default function ServiceList({ userInfo, setUserInfo }) {
  const [serviceListData, setServiceListData] = useState([]);
  const email = userInfo.email;
  useEffect(() => {
    Axios.get(`http://localhost:8000/getSingleUserOrder?email=` + email).then((res) => {
      ...
      <Row className="align-items-center justify-content-center">
        {serviceListData.length === 0 ? (
          ...
          serviceListData.map((data) => (
            <div className="col-md-5 p-3 mx-2 mb-3" key={data._id} style={{ backgroundColor: "#fff", borderRadius: "10px" }}>
              <Row className="align-items-center justify-content-center">
